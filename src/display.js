...
function printBanner(text) { console.log(chalk.cyan(text)); }
...
function logX402(agent, method, path, cost, status, latency, txid) {
  ...
  _log(`${timestamp()} ${chalk.cyan('x402')} ${chalk.cyan('◆')} ${txStr}${agent.id} → ${method} ${path}  │ ${costStr} │ 200 OK │ ${latStr}`);
}

function logKya(agent, oldScore, newScore) {
  const delta = (newScore - oldScore).toFixed(2);
  const sign  = delta >= 0 ? '+' : '';
  const arrow = newScore >= oldScore ? chalk.cyan('▲') : chalk.gray('▼');
  _log(`${timestamp()} ${chalk.cyan('kya')}  ${arrow} ${agent.id} score updated: ${oldScore.toFixed(2)} → ${newScore.toFixed(2)} ${chalk.gray(`(${sign}${delta})`)}`);
}
...
function printDashboard(agents) {
  const table = new Table({
    head: [chalk.cyan('Agent'), chalk.cyan('Role'), chalk.cyan('KYA Score'), chalk.cyan('Budget'), chalk.cyan('Used'), chalk.cyan('Status')],
    style: { head: [], border: [] },
